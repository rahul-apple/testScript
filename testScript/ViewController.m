...
- (void)viewDidLoad {
    ...
    [chatEncoder EncryptMessage:messageString onCompletion:^(NSData *encryptedData, NSString *IV) {
//        [_txtView setText:[NSString stringWithFormat:@"%@\nIV Genarated:%@",_txtView.text,IV]];
//        [_txtView setText:[NSString stringWithFormat:@"%@\nencrypted Message:%@",_txtView.text,encryptedData]];
        [chatEncoder DecryptMessage:encryptedData andIV:IV onCompletion:^(NSString *message) {
